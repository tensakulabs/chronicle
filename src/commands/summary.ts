...
import { validateProjectIndex, withDatabase, normalizePath } from '../utils/index.js';
...
export function summary(params: SummaryParams): SummaryResult {
    ...
    try {
        return withDatabase(validation.dbPath, true, (db, queries) => {
...
export function tree(params: TreeParams): TreeResult {
    ...
    try {
        return withDatabase(validation.dbPath, true, (db, queries) => {
            const files = queries.getAllFiles();
            const normalizedSubpath = subpath ? normalizePath(subpath).replace(/^\/|\/$/g, '') : '';
